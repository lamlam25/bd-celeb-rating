...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    await addRating({ userId: user.id, userEmail: user.email, userName: user.name, imageId, celebrityId, celebrityName, imageNumber, rating });
    return res.json({ success: true });
  }

  if (req.method === 'GET') {
    const ratings = await getRatingsByUser(user.id);
